...
def findMeshesNearby(cleanPts: list[Point]) -> list[Point]:
    ...
    for pt in cleanPts:
        ...
        for p in cleanPts:
            distance = np.sqrt(
                np.sum(
                    (np.array([p.x, p.y, p.z]) - np.array([pt.x, pt.y, pt.z])) ** 2,
                    axis=0,
                )
            )
            if distance == 0:
        ...
        meshIds = [
            m for i, m in enumerate(all_mesh_ids) if all_mesh_dist[i] < minDist * 2
        ]
...
def cleanPtsList(pt_origin, all_pts, usedVectors):
    ...
    for i, pt in enumerate(all_pts):
        ...
        if vectorCount > 1:
            pack = [
                [np.array([p.x, p.y, p.z]), x, p.meshId]
                for x, p in enumerate(all_pts)
                if p.vectorId == vectorId
            ]
            ...
            checkedPtIds.extend(
                competingPtIds
            )  # remove points from this vector from further search
